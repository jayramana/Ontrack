...
namespace Backend.Services
{
    public class VerificationService
    {
        ...
        // =====================================================
        // ORCHESTRATION: COMPLETE ASR VERIFICATION
        // =====================================================
        public async Task<ASRVerificationResultDto> VerifyCompleteASRAsync(
            List<string> documentUrls,
            string customerPhotoUrl, // Base64
            string signatureUrl)     // Base64
        {
            var reasons = new List<string>();
            var verified = true;
            double score = 0;
            object? aadhaarData = null;

            string? idCardBase64 = null;

            // 1. Download ID Card Image
            if (documentUrls != null && documentUrls.Any())
            {
                try
                {
                    var url = documentUrls.First();
                    // Basic validation to ensure it's a URL
                    if (Uri.IsWellFormedUriString(url, UriKind.Absolute))
                    {
                        var bytes = await _http.GetByteArrayAsync(url);
                        idCardBase64 = Convert.ToBase64String(bytes);
                        reasons.Add("✓ ID Document retrieved");
                    }
                    else
                    {
                        reasons.Add("⚠ Invalid document URL format");
                        // If local path or other issue, we might skip
                    }
                }
                catch (Exception ex)
                {
                    reasons.Add($"⚠ Failed to download ID document: {ex.Message}");
                }
            }
            else
            {
                reasons.Add("⚠ No ID document found for verification");
            }

            // 2. Perform Aadhaar OCR (if ID available) - Optional but good for data
            if (!string.IsNullOrEmpty(idCardBase64))
            {
                // We use the OCR to get data, even if we don't strictly "verify" against a database here yet
                var ocrResult = await AadhaarOcrWithGeminiAsync(idCardBase64);
                if (ocrResult != null)
                {
                    aadhaarData = ocrResult;
                    // reasons.Add("✓ Aadhaar OCR successful");
                }
            }

            // 3. Verify Face Match
            if (!string.IsNullOrEmpty(idCardBase64) && !string.IsNullOrEmpty(customerPhotoUrl))
            {
                var faceResult = await FaceMatchAsync(idCardBase64, customerPhotoUrl);
                
                // Allow some tolerance or fallback if service is down
                if (faceResult.Reason == "Face verification service unavailable")
                {
                     reasons.Add("⚠ Face verification service skipped (unavailable)");
                     // Don't fail verification if service is down? Or perform fail?
                     // Let's keep verified=true if other things pass, or make it manual check.
                }
                else if (faceResult.FaceMatch)
                {
                    reasons.Add($"✓ Face match confirmed ({faceResult.Similarity:F1}% match)");
                    score += 0.5;
                }
                else
                {
                    reasons.Add($"✗ Face verification failed: {faceResult.Reason}");
                    verified = false;
                }
            }

            // 4. Verify Signature
            if (!string.IsNullOrEmpty(idCardBase64) && !string.IsNullOrEmpty(signatureUrl))
            {
                var sigMatch = await VerifySignatureWithGeminiAsync(idCardBase64, signatureUrl);
                if (sigMatch)
                {
                    reasons.Add("✓ Signature verified with Gemini AI");
                    score += 0.5;
                }
                else
                {
                    reasons.Add("✗ Signature mismatch or unclear");
                    // Weak failure for signature? ASR requires signature so we should probably fail.
                    verified = false; 
                }
            }
            ...
            if (string.IsNullOrEmpty(idCardBase64)) 
            {
                verified = false; // Cannot verify without ID
            }

            return new ASRVerificationResultDto
            {
                Verified = verified && score >= 0.5,
                Score = score,
                Reasons = reasons,
                AadhaarData = aadhaarData,
                VerificationType = "Gemini+DeepFace"
            };
        }
    }
}