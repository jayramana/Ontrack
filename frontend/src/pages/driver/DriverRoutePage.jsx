...
// const ROUTE_MODES = [
//   { id: "Balanced", label: "🎯 Balanced", desc: "Priority + Distance" },
//   { id: "PriorityFirst", label: "⭐ Priority", desc: "High priority first" },
//   { id: "TimeWindow", label: "⏰ Time Window", desc: "Rescheduled first" },
//   { id: "AvoidIssues", label: "🚧 Avoid Issues", desc: "Avoid road issues" },
// ];
...
//    DRIVER ICON
// =========================== */
// const driverArrowIcon = (heading = 0) =>
//   L.divIcon({
//     html: `<div style="transform:rotate(${heading}deg);font-size:26px;">➤</div>`,
//     iconSize: [26, 26],
//     className: "",
//   });
...
// const stopIcon = (num, priority) => {
//   const color =
//     priority >= 4 ? "#dc2626" : priority >= 3 ? "#f59e0b" : "#2563eb";
//   return L.divIcon({
//     html: `<div style="
//       background:${color};
//       color:white;
...
//    MAP FITTER
// =========================== */
// function MapBoundsFitter({ coords, stops, driver }) {
//   const map = useMap();
...
//     coords.forEach(([lat, lng]) => isValidTN(lat, lng) && pts.push([lat, lng]));
//     stops.forEach(
//       (s) => isValidTN(s.lat, s.lng) && pts.push([s.lat, s.lng])
//     );
...
//   useEffect(() => {
//     if (!driverPos || !lastDriverPosRef.current) {
...
//     const startLat =
//       driverPos?.lat || rawStops[0].lat || DEFAULT_CENTER[0];
//     const startLng =
//       driverPos?.lng || rawStops[0].lng || DEFAULT_CENTER[1];
...
//     setStops(ordered);
//     await calculateRoute(ordered, startLat, startLng);
...
//   const calculateRoute = async (orderedStops, lat, lng) => {
//     abortRef.current?.abort?.();
//     const controller = new AbortController();
//     abortRef.current = controller;
...
//     const base = Math.round(route.duration / orderedStops.length / 60);
//     setEtas(orderedStops.map((_, i) => base * (i + 1)));

//     setNavSteps(
//       route.legs.flatMap((l) =>
//         l.steps.map((s) => ({
//           text: `${s.maneuver.type} ${s.maneuver.modifier || ""} onto ${
//             s.name
//           }`,
...
//       {routeChanged && (
//         <div className="fixed top-16 right-6 z-50 bg-green-100 border px-4 py-2 rounded shadow">
//           🔄 Route Updated
//         </div>
//       )}
...
//                   mode === m.id ? "bg-yellow-100" : "bg-white"
//                 }`}
//               >
//                 {m.label}
//                 <div className="text-xs text-gray-500">{m.desc}</div>
...
//           {/* MAP */}
//           <div className="lg:col-span-3 relative h-[520px] bg-white shadow rounded">
//             <MapContainer
//               center={DEFAULT_CENTER}
//               zoom={12}
//               style={{ height: "100%" }}
...
//                   icon={driverArrowIcon(driverPos.heading)}
//                 >
//                   <Tooltip>You</Tooltip>
//                 </Marker>
//               )}
...
//             {/* NAVIGATION */}
//             <div className="absolute bottom-4 right-4 bg-white shadow-lg rounded p-3 w-72">
//               <h4 className="font-semibold mb-2">🧭 Navigation</h4>
//               {navSteps.slice(0, 3).map((s, i) => (
...
//           <h4 className="font-semibold mb-2">🚧 Active Road Issues</h4>
//           {roadIssues.length === 0 && (
