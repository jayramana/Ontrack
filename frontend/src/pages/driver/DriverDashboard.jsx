...
export default function DriverDashboard() {
  ...
  // 🆕 Open ASR Verification
  const openASRVerification = (id) => {
  ...
  const setupSignalR = async () => {
    ...
    try {
      ...
      conn.on("CustomerDocumentsUploaded", (data) => {
        alert(`Customer uploaded documents for Order #${data.orderId}`);
        fetchTodaysOrders();
      });

      conn.on("ASRVerificationCompleted", (data) => {
        const status = data.status === "Success" ? "✅ Verified" : "❌ Failed";
        alert(`ASR ${status} for Order #${data.orderId}`);
        fetchTodaysOrders();
      });

      conn.on("ASRAdminOverride", (data) => {
        alert(`Admin approved ASR for Order #${data.orderId}: ${data.reason}`);
        fetchTodaysOrders();
      });
  ...
  const markDelivered = async (id, order) => {
    // 🆕 Check ASR status before delivery
    if (order.isASR && order.asrStatus !== "Success" && order.asrStatus !== "AdminOverride") {
      alert("⚠️ Cannot complete delivery: ASR verification required!");
      openASRVerification(id);
      return;
    }
  ...
  const renderOrder = (order, idx) => (
    <div
      key={order.id}
      className={`bg-[#fff8e7] border rounded-xl p-6 shadow hover:shadow-md transition cursor-pointer ${
        order.isASR ? "border-red-300 border-2" : "border-[#e6ddc5]"
      }`}
      onClick={() => openOrderDetails(order.id)}
    >
      {/* Header */}
      <div className="flex justify-between mb-4">
        <div>
          <h3 className="font-bold text-[#351c15] text-lg flex items-center gap-2">
            {order.isASR && <span className="text-red-600">🔒</span>}
            Stop #{idx + 1}
          </h3>
          {getPriorityBadge(order.priority)}
          {getASRBadge(order)}
      ...
      {/* 🆕 ASR Warning */}
      {order.isASR && order.asrStatus !== "Success" && order.asrStatus !== "AdminOverride" && (
        ...
        <div className="max-w-7xl mx-auto p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-6 mb-10">
            {[
              ...
              ["Normal", orders.filter((o) => o.priority === 2).length],
              ["Rescheduled", orders.filter((o) => o.priority === 3).length],
              ["🔒 ASR Required", orders.filter((o) => o.isASR).length],
            ].map(([label, count], i) => (
              <div
                key={i}
          ...
          <div className="mt-10 grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <button
              onClick={() => window.location.href = "/driver/issues"}
              className="bg-[#f9b400] hover:bg-[#e0a200] text-[#351c15] rounded-xl p-5 shadow font-semibold"
            >
              ⚠️ Report Road Issue
            </button>
