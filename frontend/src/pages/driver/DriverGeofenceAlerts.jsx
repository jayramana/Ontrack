...
export default function DriverGeofenceAlerts() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        // 2️⃣ Load all geofences
        const gfRes = await api.get("/geofence/list");
        const allGf = gfRes.data;

        const myGf = allGf.filter((g) => map[g.orderId]);
        setGeofences(myGf);

        // 3️⃣ Get driver location
        let myLocation = null;
        try {
          const meRes = await api.get(`/auth/${user.userId}`);
          myLocation = meRes.data;
        } catch {
          console.error("Failed to load driver location");
        }
        ...
        for (const gf of myGf) {
          ...
          // ❗ Prevent null or invalid coordinates → avoid 400 Bad Request
          if (
            !myLocation ||
            myLocation.currentLatitude == null ||
            myLocation.currentLongitude == null ||
            isNaN(myLocation.currentLatitude) ||
            isNaN(myLocation.currentLongitude)
          ) {
            statusMap[gf.geofenceId] = { error: "Driver location unavailable" };
            continue;
          }

          try {
            // 4️⃣ Correct payload field names (camelCase)
            const chk = await api.post("/geofence/check", {
              driverId: Number(user.userId),
              orderId: Number(order.id),
              lat: Number(myLocation.currentLatitude),
              lon: Number(myLocation.currentLongitude),
  ...
  const renderGeofenceCard = (gf) => {
    ...
    return (
      <div
        key={gf.geofenceId}
        className="bg-white border border-[#e6d8c9] rounded-xl shadow-md hover:shadow-lg transition-all duration-200"
      >
        <div className="bg-[#f8f4ef] border-b border-[#e6d8c9] p-4 flex justify-between items-center">
          <div>
            <h3 className="text-lg font-bold text-[#351c15]">{gf.name}</h3>
        ...
        <div className="p-5">
          <div className="flex justify-between items-center mb-5 text-sm">
            <div>
              <span className="text-xs text-[#6b4f3a] font-bold uppercase">Radius</span>
              <span className="font-bold text-[#351c15] text-lg">
                {gf.radiusMeters} <span className="text-xs text-[#6b4f3a]">m</span>
              </span>
            </div>
          ...
          {status ? (
            status.error ? (
              ...
              <div
                className={`p-4 rounded border-l-4 ${
                  isInside ? "bg-green-50 border-green-600" : "bg-[#fff8e6] border-[#ffb500]"
                }`}
              >
            ...
            <div className="text-center text-[#6b4f3a] py-3 text-sm italic">Finding location…</div>
          )}
        </div>
      </div>
    );
      ...
      {list.map((gf) => {
        ...
        return (
          ...
          >
            <div>
              <span className="text-lg font-bold text-[#351c15]">{gf.name}</span>
              <p className="text-xs text-[#6b4f3a]">Radius: {gf.radiusMeters}m</p>
            </div>
            ...
            <div>
              {status ? (
                <div
                  className={`px-4 py-2 rounded font-bold text-sm flex items-center ${
                    isInside ? "bg-green-100 text-green-800" : "bg-[#fff8e6] text-[#8a5a00]"
                  }`}
                >
                  <span
                    className={`h-2 w-2 rounded-full mr-2 ${
    ...
    <div className="min-h-screen flex bg-[#f8f4ef]">
      ...
      <main className="flex-1 p-8">
        <header className="mb-8 pb-6 border-b border-[#e6d8c9] flex justify-between items-end">
          <div>
            <h1 className="text-3xl font-black text-[#351c15]">Geofence Monitor</h1>
            <p className="text-[#6b4f3a] mt-1">Real-time UPS-style geofence tracking</p>
          </div>

          <div className="bg-white border border-[#e6d8c9] rounded-lg flex">
            <button
              onClick={() => setViewMode("grid")}
              className={`p-2 ${
                viewMode === "grid" ? "bg-[#ffb500]/20 text-[#351c15]" : "text-gray-500"
              }`}
            >
              ⬜⬜
            </button>
            <button
              onClick={() => setViewMode("list")}
              className={`p-2 ${
                viewMode === "list" ? "bg-[#ffb500]/20 text-[#351c15]" : "text-gray-500"
              }`}
            >
              ☰
            </button>
        ...
        {loading && geofences.length === 0 && (
          <div className="text-center py-20">
            <div className="animate-spin w-10 h-10 border-4 border-[#e6d8c9] border-t-[#351c15] rounded-full mx-auto mb-4"></div>
            <p className="text-[#6b4f3a]">Syncing Geofence Data…</p>
          </div>
        )}

        {/* ACTIVE GEOFENCES */}
        <section className="mb-10">
          <h2 className="text-xl font-bold text-[#351c15] mb-4 flex items-center">
            <span className="bg-[#ffb500]/20 text-[#8a5a00] px-2 py-1 rounded mr-2 text-sm">
              {activeGeofences.length}
            </span>{" "}
            Active Zones
          </h2>
          ...
          {activeGeofences.length ? (
            ...
            <div className="bg-white border-2 border-dashed border-[#e6d8c9] rounded-lg p-10 text-center">
              <p className="text-[#6b4f3a]">No active geofences.</p>
            </div>
          )}
        </section>
        ...
        {/* HISTORY */}
        <section>
          <h2 className="text-sm uppercase tracking-wider text-[#6b4f3a] mb-4 border-b border-[#e6d8c9] pb-2">
            Completed History
          </h2>
          ...
          {expiredGeofences.length ? (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {expiredGeofences.map((gf) => (
                <div
                  key={gf.geofenceId}
                  className="bg-[#f1e8da] border border-[#e6d8c9] rounded p-4 flex justify-between items-center opacity-70"
                >
                  <div>
                    <h4 className="font-bold text-[#351c15] text-sm">{gf.name}</h4>
                    <p className="text-xs text-[#6b4f3a] font-mono">#{gf.geofenceId}</p>
                  </div>
                  <span className="bg-green-100 text-green-800 text-xs font-bold px-2 py-1 rounded">
                    DELIVERED
                  </span>
            ...
            <p className="text-[#6b4f3a] italic text-sm">No history.</p>
          )}
        </section>
      </main>
    </div>
